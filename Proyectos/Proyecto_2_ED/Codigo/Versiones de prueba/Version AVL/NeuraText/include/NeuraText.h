...
#include "../unicode/include/unicode/utypes.h"
#include "../unicode/include/unicode/stringoptions.h"
#include "../unicode/include/unicode/ucpmap.h"
#include "../unicode/include/unicode/umachine.h"
...
private:
    AVLTreeDictionary<int, string>* MegaEstructura2 = new AVLTreeDictionary<int, string>();
    ...
    void cargarArchivoAux2(string nombre) {
        string linea;
        std::ifstream archivo(nombre);
        int contador = 0;
        if (archivo.is_open()) {
            ...
            while (archivo.good()) {
                getline(archivo, linea);
                MegaEstructura2->insert(contador,linea);
                contador++;
            }
        archivo.close();
        }
        ...
        string temp;
        int numLinea = 0;
        for (int i = 0; i < MegaEstructura2->getSize(); i++) {
            temp = MegaEstructura2->getValue(i);
            string word = "";
...
public:
    NeuraText() {
        MegaEstructura2 = new AVLTreeDictionary<int, string>();
    }
    ...
    void cargarArchivo2(string nombre) {
        cargarArchivoAux2(nombre);
